...
public class TouchMouse implements IController {
	...
	public void handleTouchMouse(View v, MotionEvent event) {
		if( mm.getPrefsHelper().isTouchMouseAsPointer() && (!Emulator.isInGame() || Emulator.isInMenu() )){
			handleTouchMouseAsPointer(v,event);
		} else {
			handleTouchMouseAsRelative(v,event);
		}
	}

	public void handleTouchMouseAsPointer(View v, MotionEvent event) {
		int pid = 0;
		...
		if (actionEvent == MotionEvent.ACTION_UP ||
			...
			for (int i = 0; i < event.getPointerCount(); i++) {
				...
				if (mouse_pid == pointerId) {

					if (!mouse_btn1_pressed && !mm.getInputHandler().getTiltSensor().isEnabled()) {
						Log.d("MOUSE", "move " + xf + " " + yf);
						if (xf >= 0 && xf <= x2 && yf >= 0 && yf <= y2) {
							Emulator.setMouseData(0, Emulator.MOUSE_MOVE_POINTER, 0, xf, yf);
							Emulator.setMouseData(0, Emulator.MOUSE_BTN_DOWN, 1, 15, 15);
							mouse_btn1_pressed = true;
						}
	...
	public void handleTouchMouseAsRelative(View v, MotionEvent event) {
		int pid = 0;
		int action = event.getAction();
		int actionEvent = action & MotionEvent.ACTION_MASK;
